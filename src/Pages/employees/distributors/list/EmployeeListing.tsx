...
const EmployeeListing = () => {
      ...
      cellRenderer: (params: any) => {
        ...
        return (
          <div className="flex mt-1 items-center gap-3">
            {checkModulePrivilegeAccess("distributors", "is_update") && (
              <Link
                to={`/admin/employees/distributors/edit/${encryptData(
                  params?.data?.id
                )}`}
              >
            ...
            <Link
              to={`/admin/employees/distributors/view/${encryptData(
                params?.data?.id
              )}`}
            >
  ...
  const fetchData = (params: any) => {
    const dataSource = {
      getRows: (param: any) => {
        ...
        axiosInstance
          .get(
            `/employees?page=${page}&search=${params?.search}&from=${params?.from}&to=${params?.to}&status=${params?.status}&type=2&size=${paginationProps?.paginationPageSize}`
          )
          .then(async (response) => {
  ...
  const downloadReport = () => {
    setIsLoading(true);
    axiosInstance
      .get(`/employees/report/download?type=2`)
      .then((response) => {
